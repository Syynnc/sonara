...
function LoginContent() {
  ...
  const handleSpotifyLogin = async (forceConsent = false) => {
    ...
    await supabase.auth.signInWithOAuth({
      ...
      options: {
        ...
        queryParams: forceConsent ? { show_dialog: 'true' } : undefined,
      },
    });
  };
  ...
  return (
    <div className="min-h-[100dvh] bg-[#121212] flex items-center justify-center px-4">
      ...
      <div className="relative w-full max-w-sm">
        ...
        {/* Reconnect banner */}
        {isReconnect && (
          <div className="mb-5 px-4 py-3 bg-[#FF5500]/8 border border-[#FF5500]/20 rounded-xl text-sm text-[#FF5500]/80 text-center leading-relaxed">
            Sonara needs updated Spotify permissions to export playlists.
            Click below to re-authorize — it only takes a second.
          </div>
        )}
        ...
        <button
          onClick={() => handleSpotifyLogin(isReconnect)}
        ...
        >
          ...
          {isReconnect ? 'Reconnect Spotify' : 'Continue with Spotify'}
        </button>
