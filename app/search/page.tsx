  ...
  return (
    <div className="min-h-[100dvh] bg-[#141414] pt-28 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        >
          <p className="text-[10px] font-bold tracking-[0.3em] text-[#d4af37]/50 uppercase mb-3">
            Discover
          </p>
        ...
        >
          <div className="relative">
            <div className="absolute left-5 top-1/2 -translate-y-1/2 text-[#d4af37]/50 pointer-events-none">
              {loading ? (
            ...
            <input
              ...
              autoFocus
              className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-2xl py-4 pl-13 pr-6 text-[#f0e6c8] placeholder:text-[#c8b87a]/35 focus:outline-none focus:border-[#d4af37]/40 transition-all duration-300 shadow-[0_8px_32px_rgba(0,0,0,0.4)] text-[15px]"
              style={{ paddingLeft: '3.25rem' }}
            />
            {query && (
              <button
                onClick={() => { setQuery(''); setResults(null); inputRef.current?.focus(); }}
                className="absolute right-4 top-1/2 -translate-y-1/2 text-[#c8b87a]/40 hover:text-[#c8b87a] transition-colors text-lg leading-none"
              >
                ×
              </button>
            )}
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {GENRE_CHIPS.map((g) => (
                <button
                  key={g}
                  onClick={() => setQuery(g)}
                  className="px-3 py-1.5 text-xs font-medium text-[#c8b87a]/60 bg-[#1a1a1a] border border-[#2a2a2a] rounded-full hover:border-[#d4af37]/35 hover:text-[#d4af37] transition-all duration-200"
                >
                  {g}
                </button>
              ))}
            ...
            {tracks.length > 0 && (
              <section style={{ animation: 'fade-up 0.4s cubic-bezier(0.16,1,0.3,1) both' }}>
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <span className="text-[10px] text-[#c8b87a]/30">{results?.tracks?.total.toLocaleString()} results</span>
                </div>
            ...
            {artists.length > 0 && (
              <section style={{ animation: 'fade-up 0.4s cubic-bezier(0.16,1,0.3,1) 0.1s both' }}>
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div className="flex-1 h-px bg-[#2a2a2a]" />
                  <span className="text-[10px] text-[#c8b87a]/30">{results?.artists?.total.toLocaleString()} results</span>
                </div>
        ...
        {searched && !loading && !hasResults && !error && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            <p className="text-4xl mb-5 text-[#c8b87a]/20 font-mono">?</p>
            <h3 className="text-lg font-semibold text-[#f0e6c8]/50 mb-2">No results for &ldquo;{query}&rdquo;</h3>
            <p className="text-sm text-[#c8b87a]/30">Try a different spelling or a broader term.</p>
          </div>
        )}
        ...
        {!searched && !loading && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <p className="text-sm text-[#c8b87a]/30 max-w-[32ch]">
